...
{
  int quad_rows = matrix_rows / 2;
  int quad_cols = matrix_columns / 2;
  ...
  for (int i = 0; i < quad_rows; i++)
  {
    ...
    {
      int q_a = matrix[i][(2 * quad_rows) - j - 1];
      int q_b = matrix[i][j];
      int q_c = matrix[(2 * quad_rows) - i - 1][j];
      int q_d = matrix[(2 * quad_rows) - i - 1][(2 * quad_rows) - j - 1];

      sum_quads += new_max(q_a, new_max(q_b, new_max(q_c, q_d)));
    }
  }
